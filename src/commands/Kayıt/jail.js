...
module.exports = {
    ...
    execute: async (client, message, args, author, channel, guild) => {
        ...
        if (member.roles.cache.has(config.registration.jailed)) return message.reply('Kullanıcı zaten kayıtsız.').then(msg => { setTimeout(() => { msg.delete() }, 10000); });
        ...
        await member.roles.set([config.registration.jailed]).catch()
        await member.setNickname(config.registration.autonickname);
        let embed = new MessageEmbed()
            .setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL({ dynamic: true }) })
            .setColor('RANDOM')
            .setDescription(`${member} kullanıcısı başarıyla kayıtsıza (<@&${config.registration.jailed}>) atıldı.`)
